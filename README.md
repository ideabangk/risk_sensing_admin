...
- **감성분석 및 라벨링**: 한국어 Rule-based 감성분석 (POSITIVE/NEGATIVE/NEUTRAL), Risk Level (CRITICAL/HIGH/MIDDLE/LOW/NONE)
- **대시보드**: 기업별/소스별 리스크 현황, 트렌드 차트, High Risk 알림
- **배치 관리**: 자동 주기 수집 (6시간), 수동 실행, 실행 이력 조회
...
- **Anthropic API** (선택, 감성분석 고도화): https://console.anthropic.com
...
업그레이드: `backend/app/services/sentiment.py`의 `analyze()` 함수를
Claude API 호출로 교체 (`.env`에 `ANTHROPIC_API_KEY` 설정)
